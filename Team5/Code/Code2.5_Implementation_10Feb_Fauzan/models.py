...
class AttentionPool(nn.Module):
    ...
    def get_entropy(weights: torch.Tensor):
    ...
    def forward(self, x: torch.Tensor, return_weights: bool = False, return_entropy: bool = False):
        ...
        Args:
            ...
            return_entropy: whether to return entropy of attention weights
        ...
        if return_weights or return_entropy:
            w = weights.squeeze(-1)  # (B, M)

        if return_weights:
            outputs.append(w)

        if return_entropy:
            entropy = self.get_entropy(w)  # (B,)
...
class HierarchicalAttnMIL(nn.Module):
    ...
    def __init__(self, base_model=None, num_classes: int = 2, embed_dim: int = 512, dropout: float = 0.3):
        ...
        if base_model is None:
            ...
            state_dict = torch.load("/projects/e32998/STAT390_Winter2026/Team5/Code/Code2_ModelTraining_05Feb_Aidan/KimiaNetPyTorchWeights.pth",
    ...
    def process_single_stain(self, slice_list: List[torch.Tensor], stain_name: str, 
        ...
        for slice_tensor in slice_list:
            ...
            slice_entropies.append(patch_entropy)
        ...
        if slice_embeddings:
            ...
            return stain_emb.squeeze(0), stain_attention_info, {"patch_entropy": torch.stack(slice_entropies).mean(), 
                                                                "stain_entropy": stain_entropy}
