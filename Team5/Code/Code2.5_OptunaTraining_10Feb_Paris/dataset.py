...
def create_transforms(is_training: bool = True) -> transforms.Compose:
    ...
    if is_training:
        ...
        transform = transforms.Compose([
            transforms.Resize((IMAGE_CONFIG['image_size'][0], IMAGE_CONFIG['image_size'][1])), # not sure if allowed larger than 224 max size
            # transforms.RandomCrop(IMAGE_CONFIG['image_size'][0]), # mistakenly changed to RandomCrop from RandomResizedCrop
            transforms.RandomHorizontalFlip(),
            transforms.RandomVerticalFlip(),
            transforms.RandomRotation(15),
            transforms.ColorJitter(brightness=0.2, contrast=0.2), # removed saturation jitter for Pres2
            transforms.ToTensor(),
