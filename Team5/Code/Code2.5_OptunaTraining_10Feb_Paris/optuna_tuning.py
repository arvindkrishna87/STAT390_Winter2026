...
Integrates with existing training pipeline to find optimal hyperparameters
"""
import os
import sys
import argparse
...
def objective(trial, train_data, val_data, args):
    ...
    trial_params = {
        ...
        'class_weights': [
            trial.suggest_float('benign_weight', 1.5, 4.0),
            1.0  # Keep high-grade at 1.0 as reference
        ],
    ...
    # Data augmentation parameters (optional)
    ...
    # Update MODEL_CONFIG for this trial
    MODEL_CONFIG['attention_hidden_dim'] = trial_params['attention_hidden_dim']
    ...
    # Prepare data loaders with trial-specific augmentation
    train_case_dict, train_label_map = train_data
    val_case_dict, val_label_map = val_data
    
    # Adjust transforms based on augmentation strength
...
def main():
    ...
    with open(config_path, 'w') as f:
        ...
        f.write(f"    'class_weights': [{study.best_params['benign_weight']}, 1.0],\n")
        f.write(f"    'scheduler_factor': {study.best_params['scheduler_factor']},\n")
        f.write(f"    'scheduler_patience': {study.best_params['scheduler_patience']},\n")
        f.write("}\n\n")
        f.write("MODEL_CONFIG = {\n")
        f.write(f"    'embed_dim': {study.best_params['embed_dim']},\n")
        f.write(f"    'attention_hidden_dim': {study.best_params['attention_hidden_dim']},\n")
        f.write("}\n")
