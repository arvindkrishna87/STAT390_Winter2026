...
EMB_DIR = "/projects/e32998/patches_pooled4096"  # <-- same as precompute script output
...
from patch_level_analysis import run_patch_level_analysis
...
def parse_args():
    ...
    # Patch-level analysis arguments
    parser.add_argument('--patch_level_analysis', action='store_true',
                    help='Compute patch-level predictions to diagnose case-level errors')
    parser.add_argument('--class_names', type=str, nargs=2,
                    default=['Benign', 'HighGrade'],
                    help='Class names in label-index order, e.g. Benign HighGrade')
...
def prepare_data(args):
    ...
    if args.load_splits:
        ...
        print("Using Optuna test cases")

        optuna_test_cases = [
        1,3,7,21,28,34,51,55,66,
        72,73,75,77,79,84,86,95,96,99
        ]

    # Select slices belonging to Optuna test cases
        test_slices = [
        key for key in slice_to_class.keys()
        if key[0] in optuna_test_cases
        ]

    # Remaining slices for train/val
        remaining_slices = [
        key for key in slice_to_class.keys()
        if key[0] not in optuna_test_cases
        ]

    # Rebuild class grouping for remaining slices
        remaining_by_class = defaultdict(list)
        for key in remaining_slices:
            label = slice_to_class[key]
            remaining_by_class[label].append(key)
...
def main():
    ...
    case_level_preds = {}
    pred_csv = "/projects/e32998/MIL_training/final_runs/run_20260301_125350/predictions.csv"
    if os.path.exists(pred_csv):
        ...
        if id_col and pred_col:
            case_level_preds = dict(zip(pred_df[id_col], pred_df[pred_col].astype(int)))

    if args.patch_level_analysis:
        test_case_dict, test_label_map = test_data
        run_patch_level_analysis(
            model            = trainer.model,
            test_case_dict   = test_case_dict,
            test_label_map   = test_label_map,
            case_level_preds = case_level_preds,
            embeddings_dir   = EMB_DIR,
            output_dir       = run_dir,
            device           = device,
    ...
    with open(results_path, 'w') as f:
        ...
        if args.patch_level_analysis:
            f.write(f"- patch_level_analysis/patch_level_predictions.csv\n")
            f.write(f"- patch_level_analysis/risky_benign_cases.csv\n")
            f.write(f"- patch_level_analysis/patch_distribution_per_case.png\n")
            f.write(f"- patch_level_analysis/patch_hg_pct_histogram.png\n")
