...
from models import create_model, AttentionPool
...
class OptunaTrainer:
    ...
    def __init__(self, model, device, trial_params, trial=None):
        ...
        # Setup loss with tuned class weights
        class_weights = torch.tensor(
            [trial_params['class_weight_benign'], 1.0], dtype=torch.float32
        ).to(self.device)
        self.criterion = nn.CrossEntropyLoss(weight=class_weights)

        # Entropy regularization coefficient
        self.entropy_coeff = trial_params.get('entropy_coeff', 0.0)
    ...
    def train_epoch(self, train_loader):
        ...
        for batch in train_loader:
            ...
            if self.entropy_coeff > 0:
                outputs, attn_weights = self.model(stain_slices, return_attn_weights=True)
                case_entropy = AttentionPool.compute_entropy(attn_weights['case_weights'])
            else:
            ...
            if self.entropy_coeff > 0:
                # Reward high-entropy (diverse) attention; mirrors trainer.py convention
                loss = ce_loss - self.entropy_coeff * case_entropy
            else:
                loss = ce_loss
    ...
    def train_with_pruning(self, train_loader, val_loader, n_epochs):
        ...
        for epoch in range(n_epochs):
            ...
            # Track best validation loss and the epoch it occurred
            if val_loss < self.best_val_loss:
                self.best_val_loss = val_loss
                best_val_acc = val_acc
                best_epoch = epoch + 1  # 1-indexed for readability
        ...
        return self.best_val_loss, best_val_acc, best_epoch
...
def objective(trial, train_data, val_data, args):
    ...
    trial_params = {
        # (a) Initial learning rate fed into ReduceLROnPlateau scheduler
        'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True),
        # (b) Weight decay
        'weight_decay': trial.suggest_float('weight_decay', 1e-6, 1e-3, log=True),
        # (c) Dropout in the patch projector
        'coeff_dropout': trial.suggest_float('coeff_dropout', 0.0, 0.5),
        # (d) Dropout in the classifier head
        'classifier_dropout': trial.suggest_float('classifier_dropout', 0.0, 0.5),
        # (e) Class weight for benign class (malignant fixed at 1.0)
        'class_weight_benign': trial.suggest_float('class_weight_benign', 1.0, 5.0),
        # (f) Entropy regularization coefficient for attention diversity
        'entropy_coeff': trial.suggest_float('entropy_coeff', 0.0, 0.5),

        # Fixed scheduler values
        'use_scheduler': True,
    ...
    # Create model with tuned classifier dropout; patch projector dropout set separately
    model = create_model(
        num_classes=MODEL_CONFIG['num_classes'],
        embed_dim=MODEL_CONFIG['embed_dim'],
        dropout=trial_params['classifier_dropout']
    )
    # Replace patch projector dropout (index 2: Linear=0, ReLU=1, Dropout=2)
    model.patch_projector[2] = nn.Dropout(trial_params['coeff_dropout'])
    ...
    # Create datasets (uses precomputed pooled features — no image transforms needed)
    ...
    # Train with pruning
    try:
        best_val_loss, best_val_acc, best_epoch = trainer.train_with_pruning(
    ...
    # Store extra info for post-hoc analysis
...
def parse_args():
    ...
    parser.add_argument('--embeddings_dir', type=str, default='/projects/e32998/patches_pooled4096',
                       help='Path to precomputed pooled feature (.pt) files')

    # Optuna arguments
    parser.add_argument('--n_trials', type=int, default=50,
                       help='Number of Optuna trials')
    parser.add_argument('--n_epochs', type=int, default=30,
                       help='Minimum number of epochs per trial')
...
def save_study_results(study, output_dir):
    ...
    stats = {
        ...
        'best_val_accuracy': study.best_trial.user_attrs.get('best_val_accuracy', None),
...
def main():
    ...
    output_dir = os.path.join('/gpfs/projects/e32998/STAT390_Winter2026/Team5/Code/Code7_OptunaChanges_27Feb_Paris', f"optuna_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)
    ...
    study = optuna.create_study(
        ...
        pruner=optuna.pruners.MedianPruner(
            ...
            n_warmup_steps=6,    # Wait 6 epochs before pruning
            interval_steps=1
        ),
    ...
    with open(config_path, 'w') as f:
        ...
        f.write(f"    'learning_rate': {bp['learning_rate']},\n")
        f.write(f"    'weight_decay': {bp['weight_decay']},\n")
        f.write(f"    'coeff_dropout': {bp['coeff_dropout']},        # patch projector dropout\n")
        f.write(f"    'dropout': {bp['classifier_dropout']},          # classifier head dropout\n")
        f.write(f"    'class_weights': [{bp['class_weight_benign']}, 1.0],\n")
        f.write(f"    'entropy_coeff': {bp['entropy_coeff']},\n")
        f.write("}\n")
