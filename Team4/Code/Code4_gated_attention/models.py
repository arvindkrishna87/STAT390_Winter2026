...
class GatedAttentionPool(nn.Module):
    ...
    a = softmax( w^T ( tanh(Vx) ⊙ sigmoid(Ux) ) )
    """
    ...
    def forward(self, x: torch.Tensor, return_weights: bool = False):
        # x: (B, M, D)
        Vx = torch.tanh(self.V(x))          # (B, M, H)
        Ux = torch.sigmoid(self.U(x))       # (B, M, H)
        H  = self.dropout(Vx * Ux)          # (B, M, H)

        scores = self.w(H)                  # (B, M, 1)
...
class HierarchicalAttnMIL(nn.Module):
    ...
    def __init__(
        ...
        # Attention modules
        self.patch_attention = GatedAttentionPool(embed_dim, MODEL_CONFIG["attention_hidden_dim"], dropout=dropout)
        self.stain_attention = GatedAttentionPool(embed_dim, MODEL_CONFIG["attention_hidden_dim"], dropout=dropout)
        self.case_attention  = GatedAttentionPool(embed_dim, MODEL_CONFIG["attention_hidden_dim"], dropout=dropout)
