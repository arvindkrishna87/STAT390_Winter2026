...
class MILTrainer:
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch in tqdm(train_loader, desc="Training", leave=False):
            ...
            entropies = []
            if want_attn:
                if attn is None or "case_weights" not in attn:
                ...
                # Optional deeper entropies if present
            ...
            entropy_term = torch.stack(entropies).mean() if len(entropies) > 0 else torch.zeros(1, device=base_loss.device, dtype=base_loss.dtype).squeeze()
    ...
    def save_loss_curve(self, output_dir: str) -> str:
        import matplotlib.pyplot as plt

        self._ensure_dir(output_dir)

        if len(self.train_losses) == 0 and len(self.val_losses) == 0:
            print("[WARN] No loss history; skipping loss curve.")
            return ""

        plt.figure()
        if len(self.train_losses) > 0:
            plt.plot(range(1, len(self.train_losses) + 1), self.train_losses, label="train")
        if len(self.val_losses) > 0:
            plt.plot(range(1, len(self.val_losses) + 1), self.val_losses, label="val")
    ...
    def evaluate(
        ...
        if save_predictions:
            ...
            self._save_deviation_plot(results, out_dir)
    ...
    def _save_deviation_plot(self, results: Dict[str, Any], output_dir: str) -> str:
        import numpy as np
        import matplotlib.pyplot as plt

        self._ensure_dir(output_dir)

        y_true = np.array(results["true_labels"], dtype=float)
        probs = np.array(results["prediction_probs"], dtype=float)

        if len(y_true) == 0:
            print("[WARN] No samples; skipping deviation plot.")
            return ""

        # For binary, use probability of class 1 (high-grade)
        if probs.ndim != 2 or probs.shape[1] < 2:
            print("[WARN] prediction_probs not in expected shape; skipping deviation plot.")
            return ""

        p1 = probs[:, 1]
        deviation = p1 - y_true  # signed error relative to label (0/1)

        plt.figure(figsize=(10, 4))
        plt.axhline(0, linewidth=1)
        plt.scatter(np.arange(len(deviation)), deviation, s=12)
        plt.xlabel("Test case index")
        plt.ylabel("p(class=1) - y")
        plt.title("Deviation Plot (Test)")
        out_path = os.path.join(output_dir, "deviation_plot.png")
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"Deviation plot saved to: {out_path}")
        return out_path
