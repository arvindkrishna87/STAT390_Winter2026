    ...
    def __init__(
        ...
        if OPTUNA_params.get('use_scheduler', True):
            self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer,
                mode='min',
                factor=OPTUNA_params.get('scheduler_factor',0.2139),
                patience=OPTUNA_params.get('scheduler_patience', 4),
...
def objective(
    ...
    # ========================================================================
    OPTUNA_CONFIG = {
        # (a) Initial learning rate — wide exploration on log scale
        'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True),

        # (b) Weight decay — wide exploration on log scale
        'weight_decay': trial.suggest_float('weight_decay', 1e-5, 1e-2, log=True),

        # (c) Project dropout (patch projector) — keeping low as intended
        'project_dropout': trial.suggest_float('project_dropout', 0.05, 0.20),

        # (d) Classifier dropout — keeping low as intended
        'classifier_dropout': trial.suggest_float('classifier_dropout', 0.05, 0.20),

        # (e) Class weight benign — allow range around starting value of 2.5285
        'class_weight_benign': trial.suggest_float('class_weight_benign', 1.5, 3.5),

        # (f) Entropy coefficient — log scale, wide range
    ...
    print(f"  Classifier dropout: {OPTUNA_CONFIG['classifier_dropout']:.3f}") # Print classifier dropout
    print(f"  Projector dropout: {OPTUNA_CONFIG['project_dropout']:.3f}")   # Print projector dropout       
    ...
    model = create_model(
        ...
        project_dropout=OPTUNA_CONFIG['project_dropout']
    )
