...
def objective(
    ...
    # ========================================================================
   OPTUNA_CONFIG = {
        # Current best: 2.08e-4 — search a wide log-scale range around it
        'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True),

        # Current best: 3.05e-4 — log scale, allow heavier or lighter regularization
        'weight_decay': trial.suggest_float('weight_decay', 1e-5, 1e-2, log=True),

        # Current best: 0.2599 — try from light to aggressive dropout
        'dropout': trial.suggest_float('dropout', 0.1, 0.5),

        # Current best: 2.5285 — benign class is minority, so allow boosting it further
        'class_weight_benign': trial.suggest_float('class_weight_benign', 1.5, 4.0),

        # Current best: 1e-3 — this is an entropy regularizer, log scale makes sense
        'entropy_lambda': trial.suggest_float('entropy_lambda', 1e-5, 1e-1, log=True),

        # Current best: 1e-6 — floor of the cosine scheduler, keep it small
