...
def attention_entropy(w: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    ...
    """
    # Softmax outputs should already be >0, 
    # but eps prevents log(0) if something odd happens.
    w = w.clamp_min(eps)
...
class MILTrainer:
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch in tqdm(train_loader, desc="Training"):
            case_data = batch[0]  # batch_size=1
            ...
            # ----- Forward pass -----
            if lam > 0.0:
                outputs, attn = self.model(stain_slices, return_attn_weights=True)
            else:
                outputs = self.model(stain_slices)
                attn = None

            # outputs: [num_classes] -> [1, num_classes]
            ...
            # Slide instruction: Loss = CE - lambda * H(w)
