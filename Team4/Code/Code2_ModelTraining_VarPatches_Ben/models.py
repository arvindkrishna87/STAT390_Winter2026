...
class HierarchicalAttnMIL(nn.Module):
    ...
    def __init__(self, base_model=None, num_classes: int = 2, embed_dim: int = 512, dropout: float = 0.3):
        ...
        if base_model is None:
            ...
            state_dict = torch.load("/home/bdp1083/Code2_ModelTraining_12Feb_Ben/KimiaNet.pth",
        ...
        self.features.eval()  # Set to eval mode to disable dropout/batchnorm updates
        
        # Adaptive pooling to get richer features than just 1x1
        #self.pool = nn.AdaptiveAvgPool2d((2, 2))
        self.pool = nn.AdaptiveAvgPool2d((1,1)) # Keep as 1x1 for now, but can experiment with larger pooling. Change
        
        # Patch projector: maps CNN features to patch embeddings with dropout
        self.patch_projector = nn.Sequential(
            nn.Linear(base_model.classifier.in_features, embed_dim),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
    ...
    def train(self, mode=True):
        """Keep frozen features in eval mode"""
        super().train(mode)
        self.features.eval()
        return self

    def process_single_stain(self, slice_list: List[List[torch.Tensor]], stain_name: str, 
                            return_attn_weights: bool = False):
        """
        Process a single stain sequentially to save memory
        Potential change to process each patch individually within this function to further reduce memory usage, and have variabel pathch size handling. 
        """
        slice_embeddings = []
        ...
        # slice_data is List[Tensor(C, H, W)] - LIST of individual tensors
        
            patch_embeddings_list = []
        
            # Process EACH patch individually (loop added!)
            for patch_tensor in slice_data:
                # patch_tensor: (C, H, W) - single patch, variable size
                patch_tensor = patch_tensor.unsqueeze(0).to(next(self.parameters()).device)  # (1, C, H, W)
            
                with torch.no_grad():
                    patch_features = self.features(patch_tensor)  # (1, F, h, w)
                    pooled = self.pool(patch_features).view(1, -1)  # (1, 4*F)
            
                patch_emb = self.patch_projector(pooled)  # (1, D)
                patch_embeddings_list.append(patch_emb)
        
        # Then concatenate all patches for this slice
            patch_embeddings = torch.cat(patch_embeddings_list, dim=0)  # (P, D)    
    ...
    def forward(self, stain_slices_dict: Dict[str, List[List[torch.Tensor]]], 
                return_attn_weights: bool = False):
        """
