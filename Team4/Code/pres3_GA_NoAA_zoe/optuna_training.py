...
from dataset import StainBagCasePooledFeatureDataset as StainBagCaseDataset, case_collate_fn
...
class OptunaTrainer:
    ...
    def attention_entropy(self, w: torch.Tensor, eps: float = 1e-12):
        w = w.clamp_min(eps)
        H = -(w * w.log()).sum(dim=-1)
        return H.mean()
        ...
        for batch in train_loader:
            ...
            if want_attn:
                try:
                    ...
                    raise TypeError("entropy_lambda>0 but model.forward does not accept return_attn_weights=True") from e

                if not (isinstance(out, tuple) and len(out) == 2):
                    raise RuntimeError("entropy_lambda>0 but model did not return (logits, attn_dict)")
                outputs, attn = out
            else:
            ...
            base_loss = self.criterion(outputs, label)

            # ---- Entropy Regularization ----
            entropies = []
            if want_attn:
                if attn is None or "case_weights" not in attn:
                    raise RuntimeError("Entropy reg enabled but attention weights not returned correctly.")

                entropies.append(self.attention_entropy(attn["case_weights"]))

                for stain, d in attn.get("stain_weights", {}).items():
                    if "slice_weights" in d:
                        entropies.append(self.attention_entropy(d["slice_weights"]))
                    for pw in d.get("patch_weights", []):
                        entropies.append(self.attention_entropy(pw))
...
def prepare_data(args):
    ...
    # sanity check one case
    some_case = next(iter(train_case_dict))
    print("Example case:", some_case)
    print("Stains:", train_case_dict[some_case].keys())
    some_stain = next(iter(train_case_dict[some_case]))
    print("Num slices in stain:", len(train_case_dict[some_case][some_stain]))
    print("Example slice length:", len(train_case_dict[some_case][some_stain][0]))
...
def create_data_loaders(train_data, val_data, per_slice_cap, max_slices_per_stain, args):
    ...
    # NOTE: pooled-feature dataset does NOT use image transforms
...
def parse_args():
    ...
    parser.add_argument("--embeddings_dir", type=str, required=True,
                        help="Directory containing precomputed pooled feature .pt files"
)
