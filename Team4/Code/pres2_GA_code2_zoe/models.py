...
class AttentionPool(nn.Module):
    ...
    Computes attention scores for instances and returns weighted bag embedding.
    """
    def __init__(self, input_dim: int, hidden_dim: int = 128, dropout: float = 0.0):
        super().__init__()

        # Two parallel projections (gated)
    ...
    def forward(self, x: torch.Tensor, return_weights: bool = False):
        ...
        Returns:
            weighted_x: (B, D)
            weights: (B, M) if return_weights=True
        """
        v = torch.tanh(self.V(x))          # (B, M, H)
        u = torch.sigmoid(self.U(x))       # (B, M, H)
        vu = self.dropout(v * u)           # (B, M, H)
