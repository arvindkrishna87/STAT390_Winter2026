...
class MILTrainer:
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch in tqdm(train_loader, desc="Training"):
            ...
            if lam > 0 and attn is not None:
                entropy_term = entropy_term + attention_entropy(attn["case_weights"])

                for stain, d in attn.get("stain_weights", {}).items():
                    entropy_term = entropy_term + attention_entropy(d["slice_weights"])
                    for pw in d.get("patch_weights", []):
                        entropy_term = entropy_term + attention_entropy(pw)
