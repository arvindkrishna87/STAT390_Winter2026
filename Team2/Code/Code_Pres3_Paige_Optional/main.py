...
def main():
    ...
    # test_data from prepare_data() 
    test_case_dict, test_label_map = test_data
    # Running the patch-level inference using trained classifier weights
    # Looping through all test patches, applies predict_patches() and saves patch_predictions.csv
    patch_results = trainer.evaluate_patches(
    ...
    # Building lookup of case-level predictions for summary
    case_pred_lookup = dict(zip(test_results["case_ids"], test_results["predictions"]))
    # Saving summary aggregating patch predictions to case level and computing benign vs high-grade %s
    patch_summary = trainer.summarize_patch_predictions_by_case(
        patch_results=patch_results,
        output_dir=run_dir,
        case_predictions=case_pred_lookup,
    )
    # Saving summary filtering to false positives 
    trainer.identify_misclassified_benign_cases(  
