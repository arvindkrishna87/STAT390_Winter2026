"""
Model architectures for MIL training for use in Optuna code
"""
import torch
import os
...
class HierarchicalAttnMIL(nn.Module):
   ...
   def __init__(self, base_model=None, num_classes: int = 2, embed_dim: int = 512, dropout: float = 0.3, attention_hidden_dim=128,):
       ...
       self.patch_attention = AttentionPool(embed_dim, attention_hidden_dim, dropout=dropout) # edited from MODEL_CONFIG['attention_hidden_dim'] so it can accept/use the parameter
       self.stain_attention = AttentionPool(embed_dim, attention_hidden_dim, dropout=dropout)# edited from MODEL_CONFIG['attention_hidden_dim'] so it can accept/use the parameter
       self.case_attention = AttentionPool(embed_dim, attention_hidden_dim, dropout=dropout)# edited from MODEL_CONFIG['attention_hidden_dim'] so it can accept/use the parameter
...
def create_model2(num_classes: int = None, 
                  embed_dim: int = None,
                  dropout: float = None, 
                  attention_hidden_dim: int = None, #added
                  pretrained: bool = True) -> HierarchicalAttnMIL:
    """
    ...
    if attention_hidden_dim is None: #added
       attention_hidden_dim = MODEL_CONFIG['attention_hidden_dim'] #added 
    ...
    model = HierarchicalAttnMIL(
        ...
        embed_dim=embed_dim,
        dropout=dropout,
        attention_hidden_dim=attention_hidden_dim # added
    )
