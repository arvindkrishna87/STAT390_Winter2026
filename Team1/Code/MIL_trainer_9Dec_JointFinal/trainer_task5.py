...
class MILTrainer:
    ...
    def __init__(self, model: nn.Module, device: str = None, checkpoint_dir: str = None):
        ...
        # Attention entropy regularization (optional add on)
        self.attn_entropy_enable = TRAINING_CONFIG.get("attn_entropy_enable", False)
        self.attn_entropy_lambda = TRAINING_CONFIG.get("attn_entropy_lambda", 1e-3)
        self.attn_entropy_eps = TRAINING_CONFIG.get("attn_entropy_eps", 1e-8)

    def _collect_attention_tensors(self, obj):
        tensors = []
        if torch.is_tensor(obj):
            tensors.append(obj)
        elif isinstance(obj, dict):
            for v in obj.values():
                tensors.extend(self._collect_attention_tensors(v))
        elif isinstance(obj, (list, tuple)):
            for v in obj:
                tensors.extend(self._collect_attention_tensors(v))
        return tensors

    def _entropy_from_weights(self, w: torch.Tensor) -> torch.Tensor:
        # entropy over last dimension
        if w.ndim == 1:
            w = w.unsqueeze(0)
        w = w.clamp_min(self.attn_entropy_eps)
        w = w / w.sum(dim=-1, keepdim=True).clamp_min(self.attn_entropy_eps)
        return -(w * torch.log(w)).sum(dim=-1).mean()

    def _attention_entropy(self, attn_info) -> torch.Tensor:
        tensors = self._collect_attention_tensors(attn_info)
        tensors = [t for t in tensors if torch.is_tensor(t) and t.numel() > 0]
        if not tensors:
            return torch.tensor(0.0, device=self.device)
        ents = [self._entropy_from_weights(t) for t in tensors]
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch in tqdm(train_loader, desc="Training"):
            ...
            if self.attn_entropy_enable:
                outputs, attn_info = self.model(
                    stain_slices,
                    return_attn_weights=True,
                    detach_attn_weights=False
                )
            else:
            ...
            if self.attn_entropy_enable:
                entropy = self._attention_entropy(attn_info)
                # CE - λH -> encourages higher-entropy (less peaky) attention
                loss = ce_loss - self.attn_entropy_lambda * entropy
            else:
                loss = ce_loss
